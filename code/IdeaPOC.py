...
def getLexFeatures(conllufilepath,lang, err):
    ...
    for line in fh:
        if not line == "\n" and not line.startswith("#"):
            ...
            if word.isalpha():
                if not word in ndw:
                    ndw.append(word) # add to an array of distinct words, and then add to an array its coressponding Part-Of-Speech
...
def regEval(predicted,actual):
    ...
    rmsle = np.sqrt((1/n) * sum(np.square(np.log10(predicted +1) - (np.log10(actual) +1)))) # Root Mean Squared Logarithmic Error
...
def train_onelang_classification(train_labels,train_data,labelascat=False, langslist=None):
    ...
    for vectorizer in vectorizers:
        for classifier in classifiers:
            ...
            cross_val = cross_val_score(classifier, train_vector, train_labels, cv=k_fold, n_jobs=1) #Evaluate a score by cross-validation

            #for each element in an array, the prediction that was obtained for that element when it was in the test set
            predicted = cross_val_predict(classifier, train_vector, train_labels, cv=k_fold, n_jobs=1) #Generate cross-validated estimates for each input data point
            print(cross_val)
